...
export default function UsernameAccordion() {
  ...
  const { control, getValues, register, handleSubmit, formState: { errors } } = useForm<UsernameInputs>({
    defaultValues: { username: profile?.username }
  });

  const disableOperations = mutationLoading;

  const onSubmit: SubmitHandler<UsernameInputs> = data => {
    updateProfile(data)
  }
        ...
        <Typography sx={{ color: 'text.secondary' }}>

          {
            profileLoading || profileFetching ? 'Loading...' :
              profile?.has_username ? profile?.username : 'No username set'
          }
        </Typography>
      ...
      <AccordionDetails>
        ...
        <div className='pb-5'>
          ...
          <Controller
            ...
            rules={{ required: true }}
            render={({ field }) =>
              <Input disabled={disableOperations} className={classNames({ "border-rose-500": errors.username })} {...field} placeholder="your.username" /*className="border-rose-500"*/ />
            }
          />
        ...
        {mutationError && <SaveUnsuccessfulAlert msg={"Please choose a different username."} error={mutationError} />}
